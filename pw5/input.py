...
def save_students_to_file(students):
    ...
    filepath = os.path.join(DATA_DIR, "students.txt")
...
def save_courses_to_file(courses):
    ...
    filepath = os.path.join(DATA_DIR, "courses.txt")
    with open(filepath, 'w') as f:
        for course in courses:
            f.write(f"{course.course_id},{course.course_name},{course.credits}\n")


def save_marks_to_file(marks):
    """Write marks to marks.txt"""
    filepath = os.path.join(DATA_DIR, "marks.txt")
    with open(filepath, 'w') as f:
...
def input_students(students, stdscr=None, output_module=None):
    ...
    if stdscr and output_module:
        ...
        output_module.draw_status_bar(stdscr, f"Added {num} student(s). Saved to students.txt. Press any key...")
        stdscr.getch()
    else:
        ...
        for i in range(num):
            ...
            students.append(Student(sid, name, dob))
        save_students_to_file(students)
        print(f"Added {num} student(s). Saved to students.txt.")
...
def input_courses(courses, stdscr=None, output_module=None):
    ...
    if stdscr and output_module:
        ...
        output_module.draw_status_bar(stdscr, f"Added {num} course(s). Saved to courses.txt. Press any key...")
        stdscr.getch()
    else:
        ...
        for i in range(num):
            ...
            courses.append(Course(cid, cname, credits))
        save_courses_to_file(courses)
        print(f"Added {num} course(s). Saved to courses.txt.")
...
def input_marks(students, courses, marks, stdscr=None, output_module=None):
    ...
    if stdscr and output_module:
        ...
        output_module.draw_status_bar(stdscr, f"Marks saved for {course.course_name}. Saved to marks.txt. Press any key...")
        stdscr.getch()
    else:
        if not courses or not students:
        ...
        save_marks_to_file(marks)
        print(f"\n {course.course_name} marks saved to marks.txt.")
        for student in students:
